...
Solution : Non, le code n'est pas structurée en couche. Les partie du programme sont bien spérarée dans des fichier différent. Cependant le paquet est le même pour l'ensemble de ces fichiers. Les différentes partie ne sont donc pas indépendantes. 

Ce problème de couplage peut poser des difficultées lors de l'ajout de nouvelles fonctionalitées. En effet les parties n'étant pas indépendantes il sera impossible de récupérer un élément du porjet sans avoir à modifier d'autre élément sans rapport avec la partie concernée.

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*

Le code métier ce trouve dans le fichier TodoListController.java.
Ce fichier est dans le dossier src/main/java.

*Cette règle est-elle facilement testable par un test unitaire ?*

Non la règle n'est pas facilement testable. IL n'y à pas d'injection de dépendance dans le TodoListController.java. L'injection de dépendance permets rend les fonction plus facile à tester
...
Cette ligne de commande permets la crétaion d'un conteneur Docker.

-it : interactive , keep STDIN open even if not attached

-p : publish a conteiner's port to the host. permets la saisie du port

-e : set environment variables. permets la saisie des paramètre d'environnement. 
...
Chaque composant est connecté au autre grâce à des ports. 
...
il faudra écrire des test unitaires. Il faudra également réaliser des test d'intégration. 

*Que teste-on dans ce cas ?*

On test l'ensembles des fonction.
Dans notre cas il faut tester,
Pour les classes de persistance 

- L'ajout d'un nouveau TodoItem ou d'un todoItem existant
- La récupération d'undes TodoItem
